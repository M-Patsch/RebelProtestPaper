...
keywords: "rebels, conflict, power-sharing, peace agreements, protests"
...
colnames(RgdMmFhCorrected)[colnames(RgdMmFhCorrected)=="pshare"] <- "Powersharing" 
colnames(RgdMmFhCorrected)[colnames(RgdMmFhCorrected)=="govern2"] <- "RebelGovernance" 
 colnames(RgdMmFhCorrected)[colnames(RgdMmFhCorrected)=="govwin"] <- "GovernmentVictory"
 colnames(RgdMmFhCorrected)[colnames(RgdMmFhCorrected)=="rebwin"] <- "RebelVictory"
colnames(RgdMmFhCorrected)[colnames(RgdMmFhCorrected)=="Settle"] <- "Settlement" 
 colnames(RgdMmFhCorrected)[colnames(RgdMmFhCorrected)=="coup"] <- "Coup"
 colnames(RgdMmFhCorrected)[colnames(RgdMmFhCorrected)=="inverse_cl"] <- "CivilLiberties"
colnames(RgdMmFhCorrected)[colnames(RgdMmFhCorrected)=="inverse_pr"] <- "PoliticalRights" 

NBR1 <- negbinmfx(data = RgdMmFhCorrected, protest1 ~ Powersharing + RebelGovernance + GovernmentVictory + RebelVictory + Settlement + Coup + CivilLiberties + PoliticalRights + t + t2 + t3, clustervar1 = "region")
summary(NBR1$fit)
NBR1$mfxest

Figure2 <- as.data.frame(NBR1$mfxest) %>% transmute(term = rownames(.), estimate = `dF/dx`, std.error = `Std. Err.`) %>% dotwhisker::dwplot() 


NBR2 <- negbinmfx(data = RgdMmFhCorrected, protest1 ~ Powersharing*elect + RebelGovernance + GovernmentVictory + RebelVictory + Settlement + Coup + CivilLiberties + PoliticalRights + t + t2 + t3, clustervar1 = "region")
summary(NBR2$fit)
NBR2$mfxest

FigNBR2 <- as.data.frame(NBR2$mfxest) %>% transmute(term = rownames(.), estimate = `dF/dx`, std.error = `Std. Err.`) %>% dotwhisker::dwplot() 
...
```{r hist, echo=FALSE, fig.cap="\\label{fig:hist}Rebel, fig.height=4, fig.width=7, message=FALSE, warning=FALSE}
 hist(RgdMmFhCorrected$govern,
              main="Histogram for Governance Institutions" ,
              xlab="Number of Institutions" ,
              border="light blue" ,
              col="blue") 
...
```{r dotwhisker_plot, fig.width = 7, fig.height = 4, warning = FALSE, message = FALSE, echo = FALSE, fig.cap="\\label{fig:dotwhisker_plot}Model l1:Negative Binomial for Protests Per Year"}
as.data.frame(NBR1$mfxest) %>% transmute(term = rownames(.), estimate = `dF/dx`, std.error = `Std. Err.`) %>% dotwhisker::dwplot() 
...
```{r dotwhisker_plot, fig.width = 7, fig.height = 4, warning = FALSE, message = FALSE, echo = FALSE, fig.cap="\\label{fig:dotwhisker_plot}Figure 3: Model 2 Negative Binomial with Interaction"}
as.data.frame(NBR2$mfxest) %>% transmute(term = rownames(.), estimate = `dF/dx`, std.error = `Std. Err.`) %>% dotwhisker::dwplot() 
```
